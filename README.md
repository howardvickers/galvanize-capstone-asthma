...
* [data.py](https://github.com/howardvickers) imports data from various csv files (downloaded from above-mentioned sources) and returns a single pandas dataframe.  The resulting dataset is also stored as a csv file to enable faster loading of data on future occasions.  The code includes functionality to run the model with selected states.
* [modclass.py](https://github.com/howardvickers) runs (trains and predicts) the final model. Initially it loads the data generated by data.py (either from previously stored version or by building new dataset).
